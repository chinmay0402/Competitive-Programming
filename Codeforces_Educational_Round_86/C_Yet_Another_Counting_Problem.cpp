 ...
 //START OF PROGRAM LOGIC
  test{
   ...
   for(ll i=0;i<q;i++){
    ll lf,rt;
    cin>>lf>>rt;
    ll fsthf=lf/l,scdhf=rt/l;
    ll tilllf=fsthf+max((ll)0,(fsthf-1)*(max(a,b)-1))+min(max(a,b)-1,(lf%l));
    if(fsthf>0 && lf>=max(a,b)-1)
     tilllf+=max(a,b)-1;
    ll tillrt=scdhf+max((ll)0,(scdhf-1)*(max(a,b)-1))+min(max(a,b)-1,(rt%l));
    if(scdhf>0 && rt>=max(a,b)-1)
     tillrt+=max(a,b)-1;
    ll ans=tillrt-tilllf;
    if(lf%l<=max(a,b)-1)
     ans+=1;
    ll final=rt-lf+1-ans;
    cout<<final<<"\n";
   }
   cout<<"\n";
  }
